...
####################################################################
# Guacamole 0.9.10 Install from GitHub 
#    with MySQL and Tomcat
# on a fresh install of Ubuntu 16.10 server
#
# Sebastien Galvagno
# 09/11/2016
#
...
EOF
	...
	echo $mypassword | sudo -S mv ${userHome}/config/tomcat.service /etc/systemd/system/tomcat.service
...
echo $mypassword | sudo -S mv ${userHome}/config/tomcat-users.xml ${tomcathome}/conf/tomcat-users.xml
...
echo $mypassword | sudo -S mv ${userHome}/config/tomcat8 /etc/default/tomcat8
...
function BuildGuacd()
{
	...
	if [ ! -d incubator-guacamole-server ]
	then
	...
	# patch Makefile.am because there has a deprecated function
...
EOF
	...
	echo $mypassword | sudo -S mv ${userHome}/config/guacamole.properties ${guacamolehome}/guacamole.properties
...
function BuildGuacamole()
{
	...
	# load source from github
	if [ ! -d incubator-guacamole-client ]
	then
